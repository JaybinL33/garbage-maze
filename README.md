...
창을 처음 열 때 화면에 맞는 가장 큰 32·16·8픽셀 타일을 고릅니다.
이후 창과 타일 크기는 고정입니다. 최소 타일로도 화면에 들어가지 않으면
오류를 알립니다. 미로 파일은 화면을 열기 전에 저장됩니다.
...
구분자나 이름이 빠진 줄은 줄 번호와 함께 오류를 알립니다. 중복 키는 마지막
값을 사용하고, 알 수 없는 키는 무시합니다. 줄 끝 주석은 지원하지 않습니다.
숫자·좌표 형식 오류는 해당 키 이름과 필요한 형식을 안내합니다.
...
수 있습니다.
메모리 부족은 원인을 명시해 알립니다. 그리기 자체가 실패하면 이벤트 루프를
끝내고 이미지·창을 정리한 뒤 오류 메시지와 종료 코드 1로 종료합니다.
